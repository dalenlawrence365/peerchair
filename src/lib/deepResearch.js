...
// Core CFO Circle Prospect Research Protocol runner, extracted so it can be
// called two ways: synchronously from the "Run deep research" button
// (POST /api/people/[id]/deep-research, see that route for the thin
// wrapper) and from a background task fired off the LinkedHelper webhook
// the moment someone accepts a connection request (see
// api/linkedhelper-webhook/route.js -> triggerPostConnectResearch). Both
// callers need the exact same behavior, so the logic lives here once.
//
// Returns { ok:true, note, searches_used, parse_failed? } on success (a
// "parse_failed" note still has a real narrative saved, just no
// verdict/score — treat it as a soft success, not an error), or
...
export async function runDeepResearch(sb, id) {
  ...
  if (!anthropicKey) return { ok: false, status: 500, error: "AI not configured" }
...
Do not write any text after the closing \`\`\` of that code block.`
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (attempt === MAX_ATTEMPTS) return { ok: false, status: 500, error: "AI request failed: " + (e.message || e) }
